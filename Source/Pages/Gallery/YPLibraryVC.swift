...
internal final class YPLibraryVC: UIViewController, YPPermissionCheckable {
    ...
    internal lazy var v = YPLibraryView(showAsset: YPConfig.library.displayAssetViewContainer)
    ...
    internal func fetchStoredCrop() -> YPLibrarySelection? {
        if isMultipleSelectionEnabled,
           selectedItems.contains(where: { $0.index == self.currentlySelectedIndex })
        {
            guard let selectedAssetIndex = selectedItems
                .firstIndex(where: { $0.index == self.currentlySelectedIndex })
            else {
                return nil
            }
            return selectedItems[selectedAssetIndex]
        }
        return nil
    }

    internal func hasStoredCrop(index: Int) -> Bool {
        return selectedItems.contains(where: { $0.index == index })
    }
